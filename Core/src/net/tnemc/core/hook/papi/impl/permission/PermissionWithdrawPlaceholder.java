 ...
 * @since 0.1.4.0
 */
public class PermissionWithdrawPlaceholder implements Placeholder {
  ...
  public boolean applies(final String[] params) {
    ...
    return params[0].equalsIgnoreCase("can") && params.length >= 4
           && params[1].equalsIgnoreCase("withdraw");
  }
  ...
  public @Nullable String onRequest(@Nullable final String account, @NotNull final String[] params) {
    ...
    return String.valueOf(((SharedAccount)accountOptional.get()).hasPermission(checkAccountOptional.get().getIdentifier(), MemberPermissions.WITHDRAW));
  }
}