 ...
 * @since 0.1.4.0
 */
public class PermissionDepositPlaceholder implements Placeholder {
  ...
  public boolean applies(final String[] params) {
    ...
    return params[0].equalsIgnoreCase("can") && params.length >= 4
           && params[1].equalsIgnoreCase("deposit");
  }
  ...
  public @Nullable String onRequest(@Nullable final String account, @NotNull final String[] params) {
    ...
    return String.valueOf(((SharedAccount)accountOptional.get()).hasPermission(checkAccountOptional.get().getIdentifier(), MemberPermissions.DEPOSIT));
  }
}