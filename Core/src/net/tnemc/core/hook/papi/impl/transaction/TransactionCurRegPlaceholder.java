 ...
 * @since 0.1.4.0
 */
public class TransactionCurRegPlaceholder implements Placeholder {
  ...
  public boolean applies(final String[] params) {
    ...
    return params[0].equalsIgnoreCase("tx") && params.length >= 5
           && params[4].equalsIgnoreCase("curreg");
  }
  ...
  public @Nullable String onRequest(@Nullable final String account, @NotNull final String[] params) {
    ...
    final String region = TNECore.eco().region().resolve(params[6]);
    final Optional<Currency> currency = TNECore.eco().currency().find(params[5]);
    if(currency.isEmpty() || !currency.get().regionEnabled(region)) {
      return null;
    }
    ...
    HoldingsModifier modifier = new HoldingsModifier(region, currency.get().getUid(), amount);
